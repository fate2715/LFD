...
def parse_args():
    ...
    parser.add_argument("--lambda-lfd", type=float, default=1.0, help="lambda LFD loss")
    parser.add_argument("--lfd-start-iter", type=int, default=1000, help="start iteration for LFD KL loss")
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.adapter = Adapter(128, 256).to(self.device)
        self.aux_head = SegmentHead(256, 128, train_dataset.num_class).to(self.device)
        ...
        self.lfd_loss = CriterionLFD(
            kd_temperature=args.kd_temperature,
            lambda_kd=args.lambda_lfd,
            lambda_feature=1.0,
            lambda_aux=0.1,
            start_iter=args.lfd_start_iter
        ).to(self.device)


        params_list = nn.ModuleList([])
        params_list.append(self.s_model)
        params_list.append(self.adapter)
        params_list.append(self.aux_head)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            s_feat_adapted = self.adapter(s_outputs[-1])
            ...
            # 传入adapter和aux_head
            self.lfd_loss.update_iter(iteration)
            lfd_loss, lfd_loss_dict = self.lfd_loss(
                s_outputs, t_outputs, targets, iteration,
                adapter=self.adapter, aux_head=self.aux_head, s_feat_adapted=s_feat_adapted
            )
            ...
            losses = (task_loss / task_loss.detach() + 
                     (kd_loss / kd_loss.detach() if kd_loss != 0 else torch.tensor(0.).cuda()) +
                     adv_G_loss +
                     skd_loss +
                     (cwd_fea_loss / cwd_fea_loss.detach() if cwd_fea_loss != 0 else torch.tensor(0.).cuda()) +
                     cwd_logit_loss +
                     ifv_loss +
                     at_loss +
                     fitnet_loss +
                     psd_loss +
                     csd_loss +
                     lfd_loss_dict['fea_loss'] / lfd_loss_dict['fea_loss'].detach() +
                     lfd_loss_dict['aux_loss'] +
                     lfd_loss_dict['kl_loss'])
            ...
            fea_loss_reduced = self.reduce_mean_tensor(lfd_loss_dict['fea_loss'])
            aux_loss_reduced = self.reduce_mean_tensor(lfd_loss_dict['aux_loss'])
            kl_loss_reduced = self.reduce_mean_tensor(lfd_loss_dict['kl_loss'])
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    ...
                    "|| psd_loss: {:.4f} || csd_loss: {:.4f} || fea_loss: {:.4f} || kl_loss: {:.4f} || aux_loss: {:.4f} " \
                    "|| Cost Time: {} || Estimated Time: {}".format(
                        ...
                        fea_loss_reduced.item(),
                        kl_loss_reduced.item(),
                        aux_loss_reduced.item(),
